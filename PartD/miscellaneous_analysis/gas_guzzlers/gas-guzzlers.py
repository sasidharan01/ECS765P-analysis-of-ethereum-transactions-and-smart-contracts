...
if __name__ == "__main__":
    ...
    lines_tra = spark.sparkContext.textFile("s3a://" + s3_data_repository_bucket + "/ECS765/ethereum-parvulus/transactions.csv")    
    # For testing small data set uncomment below lines and comment above line  
    ...
    address_tra = clean_lines_tra.map(lambda l: (l.split(',')[6], (time.strftime("%m/%Y",time.gmtime(int(l.split(',')[11]))),float(l.split(',')[7]),float(l.split(',')[8]),float(l.split(',')[9]))))
    ...
    print('time_gas_price_reduce----------------------------'*20, time_gas_price_reduce.collect())
    # [('08/2015', 4000000000000.0)]
    
    my_result_object = my_bucket_resource.Object(s3_bucket,'ethereum' + '/partdgasguzzlers_time_gas_price.txt')
    ...
    print('time_gas_reduce???????????????????????????????'*20, time_gas_reduce.collect())
    # [('08/2015', 168000.0)] 
    
    my_result_object = my_bucket_resource.Object(s3_bucket,'ethereum' + '/partdgasguzzlers_time_gas.txt')
    ...
    # (time, value)
    # [('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0), ('08/2015', 5000000000000000.0)]
    time_value_reduce = time_value_map.reduceByKey(operator.add)
    
    print('time_value_reduce++++++++++++++++++++++++++++++++++'*20, time_value_reduce.collect())
    # [('08/2015', 4e+16)]     
    
    my_result_object = my_bucket_resource.Object(s3_bucket,'ethereum' + '/partdgasguzzlers_time_value.txt')
    my_result_object.put(Body=json.dumps(time_value_reduce.collect()))  
    
    
    time_gas_price_count_map = con_tra_joined.map(lambda l: ('Average', (l[1][1][3], 1)))
    ...
    average_gas_price = time_gas_price_count_map.reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1])).map(lambda x: (x[0],x[1][0] / x[1][1]))
    
    print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'*30, average_gas_price.collect())
    
    my_result_object = my_bucket_resource.Object(s3_bucket,'ethereum' + '/partdgasguzzlers_average_gas_price.txt')
