...
if __name__ == "__main__":
    ...
    transactions_scams_joinedd = transactions_transformed.join(scams_transformed)

    # Transform the joined dataset to map scam types and addresses to transaction values
    transactions_scams_transformed = transactions_scams_joinedd.map(
    ...
    """
    Create a mapping between each transaction and its associated 
    ...
    """ 
    Join the two above mappings to create a new RDD that contains the scam type, 
    scam address, month and transaction value of each transaction associated 
    with a scam.
    """
    tran_month_scam_addr_joined = transaction_month_mapped.join(scam_address_mapped)

    """
    Create a new RDD that maps the month and scam address to the sum of all 
    transaction values that occurred during that month for that scam address.
    """
    ...
    obj = bucket.Object(
        s3_bucket, "ethereum_scam_analysis_" + date_time + "/ether_time.txt"
    )
    obj.put(Body=json.dumps(ethertime_time_transformed.take(100)))
